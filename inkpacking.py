...
class inkpacking(inkex.Effect):
    ...
    def __init__(self):
        ...
        self.arg_parser.add_argument(
            "--width", action="store", type=float, dest="width", default=10.0, help=""
        )
        self.arg_parser.add_argument(
            "--height",
            action="store",
        ...
        self.arg_parser.add_argument(
            "--depth", action="store", type=float, dest="depth", default=3.0, help=""
        )
        self.arg_parser.add_argument(
            "--unit", action="store", type=str, dest="unit", default="mm", help=""
        )
        self.arg_parser.add_argument(
            "--topscheme",
            action="store",
        ...
        self.arg_parser.add_argument(
            "--about", action="store", type=str, dest="about", default="", help=""
        )
        self.arg_parser.add_argument(
            "--tfal",
            action="store",
        ...
        self.arg_parser.add_argument(
            "--roto", action="store", type=float, dest="roto", default=0, help=""
        )

    def effect(self):

        # docW = self.getUnittouu(self.document.getroot().get("width"))
        docW = self.getUnittouu(self.svg.viewbox_width)
        docH = self.getUnittouu(self.svg.viewbox_height)
        ...
        fingergrepr = self.getUnittouu(
            str(self.options.fingergrepr) + self.options.unit
        )
        ...
        glueflapinoff = self.getUnittouu(
            str(self.options.glueflapinoff) + self.options.unit
        )
        glueflapin45 = self.getUnittouu(
            str(self.options.glueflapin45) + self.options.unit
        )
        ...
        glueflapindesl = (
            ((boxD + boxL) / 2 - glueflapinoff - glueflapin45)
            / math.sin(math.radians(90 - glueflapinang))
            * math.sin(math.radians(glueflapinang))
        )

        glueflapouoff = self.getUnittouu(
            str(self.options.glueflapouoff) + self.options.unit
        )
        glueflapou45 = self.getUnittouu(
            str(self.options.glueflapou45) + self.options.unit
        )
        glueflapouang = self.options.glueflapouang

        glueflapoudesl = (
            ((boxD + boxL) / 2 - glueflapouoff - glueflapou45)
            / math.sin(math.radians(90 - glueflapouang))
            * math.sin(math.radians(glueflapouang))
        )

        bglueflapinoff = self.getUnittouu(
            str(self.options.bglueflapinoff) + self.options.unit
        )
        bglueflapin45 = self.getUnittouu(
            str(self.options.bglueflapin45) + self.options.unit
        )

        bglueflapinang = self.options.bglueflapinang

        bglueflapindesl = (
            ((boxD + boxL) / 2 - bglueflapinoff - bglueflapin45)
            / math.sin(math.radians(90 - bglueflapinang))
            * math.sin(math.radians(bglueflapinang))
        )

        bglueflapouoff = self.getUnittouu(
            str(self.options.bglueflapouoff) + self.options.unit
        )
        bglueflapou45 = self.getUnittouu(
            str(self.options.bglueflapou45) + self.options.unit
        )
        bglueflapouang = self.options.bglueflapouang

        bglueflapoudesl = (
            ((boxD + boxL) / 2 - bglueflapouoff - bglueflapou45)
            / math.sin(math.radians(90 - bglueflapouang))
            * math.sin(math.radians(bglueflapouang))
        )
        ...
        angx = math.asin((boxL - thck) / lockrr)
        angy = (3.141615 / 2) - angx
        lockroff = lockrr - (lockrr * math.sin(angy))

        box_id = self.svg.get_unique_id("box")

        self.box = gtemp = etree.SubElement(self.svg.get_current_layer(), "g", {"id": box_id})

        line_style = formatStyle({ 'stroke': '#000000', 'fill': 'none' })

        gflapoffy = (gflapsize / math.sin((gflapangle / 360) * 6.28)) * math.sin(
            ((90 - gflapangle) / 360) * 6.28
        )
        ...
        line_atts = {
            "style": line_style,
            "id": box_id + "-sideglueflap",
            "d": formatPath(line_path),
        }
        etree.SubElement(gtemp, inkex.addNS("path", "svg"), line_atts)
                ...
                line_path = [
                    ...
                    [
                        ...
                        [
                            ...
                            1,
                            0,
                            boxW + boxD + boxW - boxW / 2 + fingergrepr,
                            0,
                        ],
                    ],
                    ["L", [boxW + boxD + boxW, 0]],
            ...
            if tfal:
                line_path = [
                    ...
                    [
                        ...
                        [
                            glueflapindesl,
                            ((boxD + boxL) / 2 - glueflapinoff - glueflapin45) * -1
                            + thck2,
                        ],
                    ],
                    ["M", [boxW + boxD, 0]],
                    ...
                    [
                        ...
                        [
                            glueflapoudesl * -1,
                            ((boxD + boxL) / 2 - glueflapouoff - glueflapou45) * -1
                            + thck2,
                        ],
                    ],
                    [
    ...
    def getUnittouu(self, param):
        try:
            return inkex.svg.unittouu(param)
